    ...
    try:
        ...
        if path == '/' and method == 'GET':
            return {
                ...
                'body': json.dumps({"message": "LangGraph Agent API is running on Netlify"})
            }
            ...
            return {
                ...
                'body': json.dumps({"status": "healthy", "service": "LangGraph Agent API"})
            }
        
        elif path.startswith('/agent-assistant') and method == 'POST':
            # Parse request body
...
def handler(event, context):
    ...
    try:
        # Extract HTTP method and a robust path that survives redirects/rewrites
        http_method = event.get('httpMethod', 'GET')

        headers = (event.get('headers') or {})
        raw_url = event.get('rawUrl') or headers.get('x-original-url') or headers.get('x-forwarded-url')
        parsed_path = None
        try:
            if raw_url:
        ...
        # Common headers Netlify/rewrites may set for original path
        header_path_candidates = [
            headers.get('x-original-uri'),
            headers.get('x-rewrite-original-path'),
            headers.get('x-forwarded-uri'),
            headers.get('x-netlify-original-path'),
        ]
        ...
        path = next((p for p in path_candidates if isinstance(p, str) and p.strip()), '/')

        # Normalize some known invocation patterns
        # e.g., "/.netlify/functions/index/health" -> "/health"
        if path.startswith('/.netlify/functions/index'):
            suffix = path[len('/.netlify/functions/index'):]
        ...
        # Main agent endpoint
        if http_method == 'POST' and (path == '/agent-assistant' or path == '/agent-assistant/' or path.endswith('/agent-assistant') or path.endswith('/agent-assistant/')):
            body = json.loads(event.get('body', '{}'))
